...
fn main() {
    ...
    let Ok(ffmpeg_dir) = env::var("FFMPEG_DIR") else {
        // No FFMPEG_DIR — ffmpeg-sys-the-third will surface a clearer
        // error before we run.
        return;
    };
    ...
    for entry in entries.flatten() {
        ...
        // Copy to target/{profile}/ for `cargo run` and the final .exe,
        // and to target/{profile}/deps/ so `cargo test` test binaries
        // (which run from deps/) also find the DLLs.
        for dst_dir in [profile_dir, &deps_dir] {
            let dst = dst_dir.join(name);
            if is_up_to_date(&src, &dst) {
                continue;
            }
            // Best-effort: log warnings rather than fail the build.
